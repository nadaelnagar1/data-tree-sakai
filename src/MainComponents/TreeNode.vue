<template>
  ...
  > -->
    ...
    >
        ...
        <div :id="node.key" class="node_node" :class="contentClass" @click="onClick" @touchend="onTouchEnd" :style="node.style">
            <button v-ripple type="button" class="p-tree-toggler p-link ml-2 mr-3" @click="toggle" tabindex="-1" aria-hidden="true">
                <span :class="toggleIcon"> </span
            ><i class="pi pi-folder ml-1 mr-2"></i>
            ...
            <button @click="disable()" class="px-2 bg-transparent border-0">
              <i class="pi pi-power-off"></i>
            </button>
          </span>
        </div>
        <ul v-if="hasChildren && expanded && !disabled" class="p-treenode-children" role="group">
            <TreeNode
...
export default {
    ...
    methods: {
      ...
      this.disabled = !this.disabled;
        //   let allnodes = document.getElementsByClassName('node_node');
          let allnodes = document.getElementsByClassName('p-highlight');

          // let titled = document.getnodesByClassName('p-treenode-label');
          for (var i = 0; i < allnodes.length; i++) {
        allnodes[i].style.backgroundColor="transparent";
      //   titled[i].style.color = 'yellow';
      }
          let catched_node = document.getElementById(this.node.key);
          // console.log(titled);
      //     catched_node.label.style.color="white";
      // catched_node.style.color= 'green' ;
    //   catched_node.style.color='green';

      // catched_node.style.color= 'green' ;
      if(this.flagColor){
        catched_node.style.backgroundColor = "red !important";
      }else{
        catched_node.style.backgroundColor = "transparent !important";

      }
      console.log(catched_node);
     
    //   catched_node.setAttribute('style', 'backgroundColor:red !important');

    //   console.log(this.node.key);
    },
        toggle() {
    ...
    computed: {
        ...
        toggleIcon() {
            return ['p-tree-toggler-icon pi pi-fw', this.expanded && !this.disabled ? this.node.expandedIcon || 'pi-chevron-down' : this.node.collapsedIcon || 'pi-chevron-right'];
        },
        checkboxClass() {
